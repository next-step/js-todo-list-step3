...
export default function TodoApp({ $target, teamId, memberId, todoList }) {
  ...
  this.getTodoList = async (_id) => {
    const memberTodos = await API.getMemberTodoList(
      this.state.teamId,
      this.state.memberId
    );
    memberTodos.hasOwnProperty("todoList")
      ? this.setTodos(memberTodos.todoList)
      : this.setTodos([]);
  };

  this.addTodo = async ({ contents }) => {
    try {
      const todo = await API.addMemberTodoList(
        this.state.teamId,
        this.state.memberId,
        { contents }
      );
  ...
  this.editTodo = async ({ contents, _id }) => {
    try {
      const editedTodo = await API.editMemberTodo(
        this.state.teamId,
        this.state.memberId,
        _id,
        contents
  ...
  this.setPriority = async ({ _id, priority }) => {
    try {
      const setPriorityTodo = await API.setMemberTodoPriority(
        this.state.teamId,
        this.state.memberId,
        _id,
        priority
  ...
  this.filterTodo = (type) => {
    if (type === ALL || type === "") {
      ...
      this.setStateForRendering(
        this.state.todoList.filter((todo) => todo.isCompleted)
      );
    } else if (type === ACTIVE) {
      this.setStateForRendering(
        this.state.todoList.filter((todo) => !todo.isCompleted)
      ...
      this.setStateForRendering(
        this.state.todoList.sort(
          (todoA, todoB) => todoA.priority - todoB.priority
        )
      );
    }
  };
