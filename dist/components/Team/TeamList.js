...
import { teamListTemplate, addTeamBtnTemplate, errorCallTemplate } from "../../utils/template.js";
...
export default function TeamList({ $target, teamList, addTeam, getOneTeam }) {
  ...
  this.render = () => {
    this.$teamList.innerHTML = this.state.teamList.map(({ _id, name }) => this.$teamList.innerHTML = teamListTemplate(_id, name)).join("") + addTeamBtnTemplate;
  };
